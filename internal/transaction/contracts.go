...
func (tx *Transaction) Execute(state *AccountState) error {
	...
	case TypeTransferNFT:
		...
		json.Unmarshal(tx.Data, &payload)

		return state.ApplyTransferNFT(tx.PublKey, payload.Recipient, payload.NFTID, tx.Nonce)

	case TypeMintNFT:
		var payload MintNFTPayload
		json.Unmarshal(tx.Data, &payload)

		return state.ApplyMintNFT(tx.PublKey, payload.Recipient, payload.NFTID, tx.Nonce)
